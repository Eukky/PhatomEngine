...
void Application::setIndices(std::vector<uint16_t> ind) {
    int size = ind.size();
    indices.resize(size);
...
VkInstance Application::getHandle() {
    return instance;
}

void Application::setSurface(VkSurfaceKHR sur) {
    surface = sur;
}
...
void Application::createSurface() {
    surface = mWindow->createSurface(instance);
}
